...
def control_panel():
    ...
    widths = [imgui.calc_text_size(label).x + pad for label, _, _ in shortcuts]
    total = sum(widths) + gap * (len(shortcuts) - 1)
    avail = imgui.get_content_region_avail().x
    imgui.set_cursor_pos_x(imgui.get_cursor_pos_x() + max(0.0, (avail - total) * 0.5))

    for index, ((label, command, hold), width) in enumerate(zip(shortcuts, widths)):
        if index:
            imgui.same_line()
